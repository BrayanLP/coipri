...
class ThumbnailBlob{
	...
	*/
	...
	function ThumbnailBlob($tmp, $path, $nombre_file){
	
		$this->path = $path;
		if($tmp){
			
			$this->name		= $_FILES[$nombre_file]["name"];
			$this->type		= $_FILES[$nombre_file]["type"];
			$this->tmp_name	        = $_FILES[$nombre_file]["tmp_name"];
			$this->size		= $_FILES[$nombre_file]["size"];
			$this->ruta_temp        = $tmp;
