...
class ThumbnailBlobArray{
	...
	*/
	...
	function ThumbnailBlob($tmp, $indice){			
		
		if($tmp){
			
			$this->name		= $_FILES["imagen"]["name"][$indice];
			$this->type		= $_FILES["imagen"]["type"][$indice];
			$this->tmp_name	= $_FILES["imagen"]["tmp_name"][$indice];
			$this->size		= $_FILES["imagen"]["size"][$indice];
			$this->ruta_temp= $tmp;
	...
	function CreateThumbnail($width=0, $height=0){		
		...
		if($width > 0 || $height > 0 ){
			...
			if(!empty($this->image)){		
				...
				imagejpeg($this->thumbnail, $path.'thumb_'.$this->name, 98);
