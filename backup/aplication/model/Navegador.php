...
class Navegador{
	...
	function bucleCatTrail($id_cat = 0, $id_prod = 0, $id_ec = 0){
		...
		if($id_prod > 0 && $id_ec == 0){
			...
			$this->ruta[$rx] = array(
									...
									'url'	=>	'solucionarios.php?id='.$producto->__get('_id'),
									'nombre'=>  $producto->__get('_nombre') );			
			$rx++;
		}

		if($id_ec > 0){
			$obj_c = new Categoria($id_ec, $this->_idioma); 
			$this->ruta[$rx] = array(
				'id'	=> 	$obj_c->__get('_id'),		
				'url'	=>	'solucionarios.php?id='.$obj_c->__get('_id'),
	...
	function display($id_actual=0){?>
		...
		<a href="solucionarios.php?cat=0">Categorias</a><?php 
