		...
		<fieldset id='form'> 
			...
			<form name='usuarios' method='post' action='' enctype="multipart/form-data" > 
                ...
                <?php endforeach ?>
					...
					<li><label> Password Usuario: </label><input type='text' name='password_usuario' value='<?php echo $usuario->getPassword(); ?>' class='text ui-widget-content ui-corner-all' size='59'  maxlength=20 ></li> 
	...
	public function getUsuarios(){
		...
		while($row = $query->VerRegistro()){
			$data[] = array(
				...
				'padre'			=> $row['id_parent'],
				'empresa'		=> $row['empresa_tarea'],
				'descripcion'	=> $row['descripcion_tarea'],
				'fecha_inicio'	=> $row['fecha_inicio_tarea'],
				'duracion'		=> $row['duracion_tarea'],
				'descripcion'	=> $row['fecha_registro_tarea']
			);		
		}
		return $data; 	
	}
			...
			<form name="f1" action="" method="post">		
				<table class="listado">
					...
					while($row = mysql_fetch_row($Query->Consulta_ID)){ ?>
						...
						$Query_SA=mysql_query("SELECT * FROM usuarios_secciones WHERE id_usuario=".$id." AND id_seccion=".$row[0]."");
