...
class Usuario{
	...
	public function __construct($id = 0){
		...
		if($this->_id > 0){
			...
			if($row = $query->VerRegistro()){ 
				...
				$this->_escritura 		= $row['escritura_usuario'];
	...
	public function setTareaActual($id_tarea){
				 
		$query = new Consulta("UPDATE tareas_actuales SET termino_tarea_actual = now() WHERE id_tarea_actual = '".$this->getTareaActualDeBD()."' ");
		$query = new Consulta("INSERT INTO tareas_actuales VALUES('','".$id_tarea."','".$this->_id."',now(),'')");
		$query = new Consulta("UPDATE tareas SET id_situacion = '3' WHERE id_tarea = '".$id_tarea."' ");
		
		$this->_tarea_actual = $id_tarea;
	}
	
	public function getTareaActualDeBD(){
		$sqlt = "SELECT * FROM tareas_actuales WHERE id_usuario = '".$this->_id."' ORDER BY id_tarea_actual DESC limit 0,1";
		$queryt = new Consulta($sqlt);
		$rowt = $queryt->VerRegistro();
		
		return  $rowt['id_tarea_actual'];
	}
	
	public function addComentario($id_tarea, $comentario){
		$query = new Consulta("INSERT INTO comentarios VALUES('','".$id_tarea."','".$this->_id."','".$comentario."')");
	}
	
	public function getTareaActual(){
		return $this->_tarea_actual;
	}	
}
